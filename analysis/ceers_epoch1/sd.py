...
# 'models/binned/croc' # has variable binning
...
for model, ls, color in zip(models, ['-','-.','--',':']*5, colors):
    ...
    bin_edges, bin_centres, N_tot = m.N(redshift_limits = redshift_limits_, log10L_limits = [27., 30.], dz = 0.1, dlog10L = 0.1, cosmo = cosmo)
    ...
    ax.plot(bin_centres['z'][::-1], np.log10(cn), c=color, alpha = 0.7, lw=1, ls = ls, label = rf'$\rm {l}\ [{ty}]$')
...
# ax.set_ylimits([-10, 2])
ax.set_xlim([redshift_limits[0],redshift_limits[1]-1])
ax.set_ylim(y_limits)
ax.set_xlabel(r'$\rm z$')
ax.set_ylabel(rf'$\rm log_{{10}}[N(>z)/arcmin^2]$')

fig.savefig(f'figs/sd.pdf')
